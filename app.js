...
function validate(){const required=step===1?[$('#date'),$('#time')]:step===2?[$('#flight'),$('#hotel')]:step===3?[$('#name'),$('#phone'),$('#email'),$('#passengerData')]:[];const bad=required.find(x=>!x.value.trim());if(bad){bad.focus();bad.reportValidity();return false}return true}
...
const today=new Date().toISOString().slice(0,10);$('#date').min=today;$('#returnDate').min=today;render();
